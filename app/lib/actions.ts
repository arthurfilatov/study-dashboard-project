...
const FormSchema = z.object({
  id: z.string(),
  customerId: z.string({ invalid_type_error: "Пожалуйста, выберите клиента." }),
  amount: z.coerce
    .number()
...
export async function updateInvoice(
  ...
  if (!validatedFields.success) {
    return {
      errors: validatedFields.error.flatten().fieldErrors,
      message: "Отсутствуют поля. Не удалось обновить счет-фактуру.",
    };
  }
  ...
  try {
    ...
    return { message: "Ошибка базы данных: Не удалось обновить счет-фактуру." };
  }
